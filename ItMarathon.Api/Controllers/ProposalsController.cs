...
public class ProposalsController(IProposalService proposalService, IMapper mapper) : ControllerBase
{
    ...
    {
        int count = 0;
        if ((top != 0 || skip !=0) && (orderby == null))
            Request.Query = AddAdditionalQueryOptions(Request, "$orderby", "id asc");
        var proposals = await proposalService.GetAllProposalsAsync(Request);
        if (filter != null)
            count = await CountFilteredProposals(Request);
        else
            count = proposals.Count();
        return Ok(new DataPage<ProposalDto>(proposals, count));
    }

    private async Task<int> CountFilteredProposals(HttpRequest request)
    {
        Dictionary<string, Microsoft.Extensions.Primitives.StringValues> queryCollection = new()
        {
    ...
    private QueryCollection AddAdditionalQueryOptions(HttpRequest request, string queryOption, string value)
    {
        if(!IsQueryOptionValid(queryOption))
            throw new InvalidOperationException(nameof(queryOption));
        Dictionary<string, Microsoft.Extensions.Primitives.StringValues> queryCollection = new();
        foreach (var query in Request.Query)
            queryCollection[query.Key] = query.Value;
        queryCollection[queryOption] = value;
        return new QueryCollection(queryCollection);
    }

    private bool IsQueryOptionValid(string queryOperation)
    {
        List<string> validOperations = new List<string>() { "$top", "$skip", "filter", "$orderby" };
        if (string.IsNullOrEmpty(queryOperation))
            throw new ArgumentNullException(nameof(queryOperation));
        return validOperations.Contains(queryOperation);
    }
